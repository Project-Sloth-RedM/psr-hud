...
PSRCore.Commands.Add('cash', 'Check Cash Balance', {}, false, function(source, args)
    local Player = PSRCore.Functions.GetPlayer(source)
    local cashamount = Player.PlayerData.money.cash
    TriggerClientEvent('hud:client:ShowAccounts', source, 'cash', cashamount)
end)

PSRCore.Commands.Add('bank', 'Check Bank Balance', {}, false, function(source, args)
    local Player = PSRCore.Functions.GetPlayer(source)
    local bankamount = Player.PlayerData.money.bank
    TriggerClientEvent('hud:client:ShowAccounts', source, 'bank', bankamount)
end)

PSRCore.Commands.Add("dev", "Enable/Disable developer Mode", {}, false, function(source, args)
...
RegisterNetEvent('hud:server:GainStress', function(amount)
    ...
    TriggerClientEvent('PSRCore:Notify', src, 9, Lang:t("notify.stress_gain"), 2000, 0, 'hud_textures', 'check')
end)
...
RegisterNetEvent('hud:server:RelieveStress', function(amount)
    ...
    Player.Functions.SetMetaData('stress', newStress)
    TriggerClientEvent('hud:client:UpdateStress', src, newStress)
    TriggerClientEvent('PSRCore:Notify', src, 9, Lang:t("notify.stress_removed"), 2000, 0, 'hud_textures', 'check')
end)

RegisterNetEvent('hud:server:saveUIData', function(data)
    local src = source
	-- Check Permissions
    if not PSRCore.Functions.HasPermission(src, 'admin') and not IsPlayerAceAllowed(src, 'command') then
		return
	end
...
PSRCore.Functions.CreateCallback('hud:server:getMenu', function(source, cb)
    cb(Config.Menu)
end) 

PSRCore.Functions.CreateCallback('hud:server:getRank', function(source, cb)
    local src = source
